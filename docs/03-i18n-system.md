...
某些 UI 元素需要根據資料狀態顯示不同文字：

```tsx
// 專家面對面 vs 一般休息時間
{segment.speakerIds.length > 0 
  ? t('agenda.expertBreakHint')  // 「現場諮詢，與專家面對面交流」
  : (language === 'zh-tw' 
      ? '放鬆時光，準備下一階段' 
      : 'Relax time, prepare for the next stage')
}
```

**注意：** 簡單的條件文字可以內嵌，複雜的應提取為 i18n key。
---
